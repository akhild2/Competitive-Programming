...
def find_second_largest(root_node):
    ...
    count_object = Count()
    second_largest(root_node, count_object)
    # print(answer)
    if count_object.count < 2:
        raise ValueError
    return count_object.answer


def second_largest(root_node, count_object):
    if root_node is None or count_object.count >= 2:
        return
    second_largest(root_node.right, count_object)
    count_object.count += 1
    if count_object.count == 2:
        print(root_node.value)
        count_object.answer = root_node.value
        return
    second_largest(root_node.left, count_object)
